...
parser = argparse.ArgumentParser(
    description=
    'Run Least Squares Deconvolution, most input parameters are in read from the file inlsd.dat.'
)
parser.add_argument(
    "observation",
    nargs='?',
    default='',
    help=
    'Observed spectrum file. If none is given, defaults to the value specificed in inlsd.dat'
)
parser.add_argument(
    "output",
    nargs='?',
    default='prof.dat',
    help=
    'Output file name for the LSD profile. If none is given, defaults to prof.dat'
)
parser.add_argument(
    "-m",
    "--mask",
    ...
    default='',
    help=
    'Mask for the LSD calculation. If none is given, defaults to the value specificed in inlsd.dat'
)
...
print('N1 reduced chi2 {:.4f} (chi2 {:.2f} constraints {:n} dof {:n})'.format(
    chi2N1 / (obs.wl.shape[0] - prof.npix), chi2N1, obs.wl.shape[0],
    prof.npix))
